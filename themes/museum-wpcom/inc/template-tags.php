...
function museum_paging_nav() {
	...
	?>
	<nav class="navigation paging-navigation <?php echo esc_attr( $class ); ?>" role="navigation">
...
if ( ! function_exists( 'museum_entry_taxonomies' ) ) :
/**
 * Display taxonomy info for single post pages
 */
function museum_entry_taxonomies() {
	if ( museum_categorized_blog() ) {
		// Display categories is this blog has more than 1 category
		echo '<span class="entry-categories genericon genericon-category">';
			the_category( ', ' );
		echo '</span>';
	}
	if ( has_tag() ) {
		// Display tags, if any exist
		echo ' <span class="entry-tags genericon genericon-tag">';
			the_tags( '', ', ' );
		echo '</span>';
	}
}
