...
function museum_setup() {
	...
	add_image_size( 'museum-single', 1840, 9999 );
...
function museum_widgets_init() {
	register_sidebar( array(
		'name'          => __( 'Footer Sidebar One', 'museum' ),
		'id'            => 'sidebar-1',
	...
	register_sidebar( array(
		'name'          => __( 'Footer Sidebar Two', 'museum' ),
		'id'            => 'sidebar-2',
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h1 class="widget-title">',
...
function museum_scripts() {
	...
	wp_enqueue_style( 'museum-fonts', museum_fonts_url(), array(), null );

	wp_enqueue_script( 'museum-navigation', get_template_directory_uri() . '/js/navigation.js', array( 'jquery' ), '20120206', true );
...
function museum_fonts_url() {
	...
	return esc_url_raw( str_replace( ',', '%2C', $fonts_url ) );
}

/**
 * Appends to the excerpt "more" text.
 */
function new_excerpt_more( $more ) {
	global $post;
	return $more . '<span class="read-more">' . __( 'Read Post', 'museum' ) . ' &rsaquo;</span>';
}
